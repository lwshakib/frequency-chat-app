  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative h-9 w-9 rounded-full hover:bg-sidebar-accent transition-colors">
      ...
      <PopoverContent align="end" className="w-80 p-0 shadow-2xl border-sidebar-border bg-popover text-popover-foreground rounded-xl overflow-hidden backdrop-blur-sm">
        <div className="flex items-center justify-between p-4 border-b border-sidebar-border bg-muted/30">
          ...
          {unreadCount > 0 && (
             <Button variant="ghost" size="sm" onClick={handleMarkAllRead} className="h-7 text-xs px-2 text-primary hover:text-primary/80 hover:bg-primary/10">
               Mark all as read
             </Button>
          )}
        </div>
